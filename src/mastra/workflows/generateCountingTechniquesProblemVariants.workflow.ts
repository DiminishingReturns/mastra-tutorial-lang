...
import { countingTechniquesProblemSchema } from '../../problem-bank/schemas/countingTechniques.schema';
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: countingTechniquesProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-counting-techniques-language-variants',

  inputSchema: countingTechniquesProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
You generate three language-register versions of a canonical counting techniques problem.
The problem must remain contextual and worded. Do not turn it into a bare symbolic counting exercise.
...
`;
    ...
    const response = await countingTechniquesLanguageVariantAgent.generate(
      prompt,
      {
        structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-counting-techniques-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-counting-techniques-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-counting-techniques-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-counting-techniques-readability-targets',
...
export const generateCountingTechniquesProblemVariantsWorkflow = createWorkflow(
  {
    id: 'generate-counting-techniques-problem-variants',
    inputSchema: countingTechniquesProblemSchema,
    outputSchema: workflowOutputSchema,
  },
)
