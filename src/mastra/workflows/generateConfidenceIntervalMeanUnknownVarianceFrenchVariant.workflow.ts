...
import { confidenceIntervalMeanUnknownVarianceFrenchLanguageVariantAgent } from '../agents/confidenceIntervalMeanUnknownVarianceFrenchLanguageVariantAgent';
...
import { confidenceIntervalMeanUnknownVarianceProblemSchema } from '../../problem-bank/schemas/confidenceIntervalMeanUnknownVariance.schema';
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: confidenceIntervalMeanUnknownVarianceProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-confidence-interval-mean-unknown-variance-french-language-variants',

  inputSchema: confidenceIntervalMeanUnknownVarianceProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème sur l’intervalle de confiance pour une moyenne lorsque l’écart-type de la population est inconnu.
...
`;
    ...
    const response =
      await confidenceIntervalMeanUnknownVarianceFrenchLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-confidence-interval-mean-unknown-variance-french-math-invariants',
...
- parameterPreserved: le paramètre à estimer est préservé.
...
- answerNotRevealed: l’intervalle final, la borne finale, la marge d’erreur finale, la valeur critique ou les calculs intermédiaires ne sont pas révélés.
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-confidence-interval-mean-unknown-variance-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-confidence-interval-mean-unknown-variance-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-confidence-interval-mean-unknown-variance-french-readability-targets',
...
export const generateConfidenceIntervalMeanUnknownVarianceFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-confidence-interval-mean-unknown-variance-french-variants',
    inputSchema: confidenceIntervalMeanUnknownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
