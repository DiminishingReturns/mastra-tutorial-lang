...
import { hypothesisTestProportionLanguageVariantAgent } from '../agents/hypothesisTestProportionLanguageVariantAgent';
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-proportion-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
You generate three language-register versions of a canonical hypothesis test problem for a population proportion.
...
`;
    ...
    const response =
      await hypothesisTestProportionLanguageVariantAgent.generate(prompt, {
        structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-proportion-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-proportion-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-proportion-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-proportion-readability-targets',
...
export const generateHypothesisTestProportionProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-proportion-problem-variants',
    inputSchema: hypothesisTestProportionProblemSchema,
    outputSchema: workflowOutputSchema,
  })
