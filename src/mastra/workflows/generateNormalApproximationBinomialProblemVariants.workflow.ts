...
import { languageVariantAgent } from '../agents/normalApproximationBinomialLanguageVariantAgent';
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-normal-approximation-binomial-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
Create three language versions of this normal distribution problem.
...
`;
    ...
    const response = await languageVariantAgent.generate(prompt, {
      structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-normal-approximation-binomial-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-normal-approximation-binomial-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-normal-approximation-binomial-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-normal-approximation-binomial-readability-targets',
...
export const generateNormalApproximationBinomialProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-normal-approximation-binomial-problem-variants',
    inputSchema: normalApproximationBinomialProblemSchema,
    outputSchema: workflowOutputSchema,
  })
