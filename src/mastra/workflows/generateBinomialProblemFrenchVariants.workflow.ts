...
import { binomialDistributionFrenchLanguageVariantAgent } from '../agents/binomialFrenchLanguageVariantAgent';
...
import { binomialDistributionProblemSchema } from '../../problem-bank/schemas/binomialDistribution.schema';
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: binomialDistributionProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-binomial-distribution-french-language-variants',

  inputSchema: binomialDistributionProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème sur la loi binomiale.
...
`;
    ...
    const response =
      await binomialDistributionFrenchLanguageVariantAgent.generate(prompt, {
        structuredOutput: {
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-binomial-distribution-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-binomial-distribution-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-binomial-distribution-french-readability-targets',
...
export const generateBinomialDistributionFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-binomial-distribution-french-variants',
    inputSchema: binomialDistributionProblemSchema,
    outputSchema: workflowOutputSchema,
  })
