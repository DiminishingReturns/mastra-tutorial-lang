...
const generateLanguageVariantsStep = createStep({
  id: 'generate-confidence-interval-mean-known-variance-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
Create three language versions of this confidence interval problem for estimating a population mean when the population standard deviation is known.
...
`;
    ...
    const response =
      await confidenceIntervalMeanKnownVarianceLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-confidence-interval-mean-known-variance-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-confidence-interval-mean-known-variance-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-confidence-interval-mean-known-variance-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-confidence-interval-mean-known-variance-readability-targets',
...
export const generateConfidenceIntervalMeanKnownVarianceProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-confidence-interval-mean-known-variance-problem-variants',
    inputSchema: confidenceIntervalMeanKnownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
