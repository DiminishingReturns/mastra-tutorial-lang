...
import { hypothesisTestMeanKnownVarianceLanguageVariantAgent } from '../agents/hypothesisTestMeanKnownVarianceLanguageVariantAgent';
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-mean-known-variance-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
You generate three language-register versions of a canonical hypothesis test problem for a population mean when the population standard deviation is known.
...
`;
    ...
    const response =
      await hypothesisTestMeanKnownVarianceLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-mean-known-variance-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-mean-known-variance-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-mean-known-variance-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-mean-known-variance-readability-targets',
...
export const generateHypothesisTestMeanKnownVarianceProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-mean-known-variance-problem-variants',
    inputSchema: hypothesisTestMeanKnownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
