...
import { hypothesisTestMeanUnknownVarianceLanguageVariantAgent } from '../agents/hypothesisTestMeanUnknownVarianceLnaguageAgent';
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-mean-unknown-variance-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
You generate three language-register versions of a canonical hypothesis test problem for a population mean when the population standard deviation is unknown.
...
`;
    ...
    const response =
      await hypothesisTestMeanUnknownVarianceLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-mean-unknown-variance-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-mean-unknown-variance-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-mean-unknown-variance-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-mean-unknown-variance-readability-targets',
...
export const generateHypothesisTestMeanUnknownVarianceProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-mean-unknown-variance-problem-variants',
    inputSchema: hypothesisTestMeanUnknownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
