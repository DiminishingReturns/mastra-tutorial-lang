...
import { hypothesisTestMeanUnknownVarianceFrenchLanguageVariantAgent } from '../agents/hypothesisTestMeanUnknownVarianceFrenchLanguageVariantAgent';
...
import { hypothesisTestMeanUnknownVarianceProblemSchema } from '../../problem-bank/schemas/hypothesisTestMeanUnknownVariance.schema';
...
const singleVariantValidationSchema = z.object({
  ...
  invariantChecks: z.object({
    ...
    sampleMeanPreserved: z.boolean(),
    sampleStandardDeviationPreserved: z.boolean(),
    sampleSizePreserved: z.boolean(),
    nullHypothesisValuePreserved: z.boolean(),
    alternativeDirectionPreserved: z.boolean(),
    significanceLevelPreserved: z.boolean(),
    questionTypePreserved: z.boolean(),
    unitPreserved: z.boolean(),
    populationStandardDeviationUnknownPreserved: z.boolean(),
    expectedDecisionNotRevealed: z.boolean(),
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: hypothesisTestMeanUnknownVarianceProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-mean-unknown-variance-french-language-variants',

  inputSchema: hypothesisTestMeanUnknownVarianceProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème de test d’hypothèse pour une moyenne lorsque l’écart-type de la population est inconnu.
...
`;
    ...
    const response =
      await hypothesisTestMeanUnknownVarianceFrenchLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-mean-unknown-variance-french-math-invariants',
...
- parameterPreserved: le paramètre testé est préservé.
- sampleMeanPreserved: la moyenne d’échantillon est préservée.
- sampleStandardDeviationPreserved: l’écart-type de l’échantillon est préservé.
- sampleSizePreserved: la taille de l’échantillon est préservée.
- nullHypothesisValuePreserved: la valeur de l’hypothèse nulle est préservée.
- alternativeDirectionPreserved: la direction de l’hypothèse alternative est préservée.
- significanceLevelPreserved: le niveau de signification est préservé.
- questionTypePreserved: le type de question est préservé.
- unitPreserved: l’unité de mesure est préservée, si applicable.
- populationStandardDeviationUnknownPreserved: le fait que l’écart-type de la population est inconnu est préservé.
- expectedDecisionNotRevealed: expectedDecision et evidenceStrength ne sont pas révélés ou suggérés.
- answerNotRevealed: la statistique de test finale, la valeur-p finale, la valeur critique, les degrés de liberté calculés, la décision finale ou la conclusion finale ne sont pas révélés.
- methodNotOverRevealed: la méthode de résolution n’est pas révélée trop directement.

Do not reveal or hint at reject-null / fail-to-reject-null.
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-mean-unknown-variance-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-mean-unknown-variance-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-mean-unknown-variance-french-readability-targets',
...
export const generateHypothesisTestMeanUnknownVarianceFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-mean-unknown-variance-french-variants',
    inputSchema: hypothesisTestMeanUnknownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
