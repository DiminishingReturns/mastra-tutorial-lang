...
import { hypothesisTestMeanKnownVarianceFrenchLanguageVariantAgent } from '../agents/hypothesisTestMeanKnownVarianceFrenchLanguageVariantAgent';
...
import { hypothesisTestMeanKnownVarianceProblemSchema } from '../../problem-bank/schemas/hypothesisTestMeanKnownVariance.schema';
...
const singleVariantValidationSchema = z.object({
  ...
  invariantChecks: z.object({
    ...
    populationStandardDeviationPreserved: z.boolean(),
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: hypothesisTestMeanKnownVarianceProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-mean-known-variance-french-language-variants',

  inputSchema: hypothesisTestMeanKnownVarianceProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème de test d’hypothèse pour une moyenne lorsque l’écart-type de la population est connu.
...
`;
    ...
    const response =
      await hypothesisTestMeanKnownVarianceFrenchLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-mean-known-variance-french-math-invariants',
...
- populationStandardDeviationPreserved: l’écart-type de la population est préservé.
...
- answerNotRevealed: la statistique de test finale, la valeur-p finale, la valeur critique, la décision finale ou la conclusion finale ne sont pas révélées.
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-mean-known-variance-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-mean-known-variance-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-mean-known-variance-french-readability-targets',
...
export const generateHypothesisTestMeanKnownVarianceFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-mean-known-variance-french-variants',
    inputSchema: hypothesisTestMeanKnownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
