...
const generateLanguageVariantsStep = createStep({
  id: 'generate-confidence-interval-mean-unknown-variance-language-variants',
  ...
  execute: async ({ inputData }) => {
    const prompt = `
Create three language versions of this confidence interval problem for estimating a population mean when the population standard deviation is unknown.
...
`;
    ...
    const response =
      await confidenceIntervalMeanUnknownVarianceLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-confidence-interval-mean-unknown-variance-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-confidence-interval-mean-unknown-variance-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-confidence-interval-mean-unknown-variance-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-confidence-interval-mean-unknown-variance-readability-targets',
...
export const generateConfidenceIntervalMeanUnknownVarianceProblemVariantsWorkflow =
  createWorkflow({
    id: 'generate-confidence-interval-mean-unknown-variance-problem-variants',
    inputSchema: confidenceIntervalMeanUnknownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
