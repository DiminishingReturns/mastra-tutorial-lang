...
import { confidenceIntervalMeanKnownVarianceFrenchLanguageVariantAgent } from '../agents/confidenceIntervalMeanKnownVarianceFrenchLanguageVariantAgent';
...
import { confidenceIntervalMeanKnownVarianceProblemSchema } from '../../problem-bank/schemas/confidenceIntervalMeanKnownVariance.schema';
...
const singleVariantValidationSchema = z.object({
  ...
  invariantChecks: z.object({
    ...
    marginOfErrorPreserved: z.boolean(),
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: confidenceIntervalMeanKnownVarianceProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-confidence-interval-mean-known-variance-french-language-variants',

  inputSchema: confidenceIntervalMeanKnownVarianceProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème sur la loi normale.
...
`;
    ...
    const response =
      await confidenceIntervalMeanKnownVarianceFrenchLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-confidence-interval-mean-known-variance-french-math-invariants',
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-confidence-interval-mean-known-variance-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-confidence-interval-mean-known-variance-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-confidence-interval-mean-known-variance-french-readability-targets',
...
export const generateConfidenceIntervalMeanKnownVarianceFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-confidence-interval-mean-known-variance-french-variants',
    inputSchema: confidenceIntervalMeanKnownVarianceProblemSchema,
    outputSchema: workflowOutputSchema,
  })
