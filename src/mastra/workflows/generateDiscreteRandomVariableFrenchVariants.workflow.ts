...
import { discreteRandomVariableFrenchLanguageVariantAgent } from '../agents/discreteRandomVariableFrenchLanguageVariantAgent';
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: discreteRandomVariableProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-discrete-random-variable-french-language-variants',

  inputSchema: discreteRandomVariableProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème de variable aléatoire discrète.
...
`;
    ...
    const response =
      await discreteRandomVariableFrenchLanguageVariantAgent.generate(prompt, {
        structuredOutput: {
...
- Les valeurs numériques sont-elles préservées ?
- Le mode de question (verbal ou tableau) est-il préservé ?
- Le type de question (calcul de probabilité, espérance, variance, etc.) est-il préservé ?
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-discrete-random-variable-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-discrete-random-variable-french-readability-targets',
...
export const generateDiscreteRandomVariableFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-discrete-random-variable-french-variants',
    inputSchema: discreteRandomVariableProblemSchema,
    outputSchema: workflowOutputSchema,
  })
