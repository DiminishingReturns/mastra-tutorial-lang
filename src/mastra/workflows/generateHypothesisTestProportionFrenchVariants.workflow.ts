...
import { hypothesisTestProportionFrenchLanguageVariantAgent } from '../agents/hypothesisTestProportionFrenchLanguageVariantAgent';
...
import { hypothesisTestProportionProblemSchema } from '../../problem-bank/schemas/hypothesisTestProportion.schema';
...
const singleVariantValidationSchema = z.object({
  ...
  invariantChecks: z.object({
    ...
    populationPreserved: z.boolean(),
    successCountPreserved: z.boolean(),
...
const generatedVariantsWithCanonicalProblemSchema = z.object({
  canonicalProblem: hypothesisTestProportionProblemSchema,
...
const generateLanguageVariantsStep = createStep({
  id: 'generate-hypothesis-test-proportion-french-language-variants',

  inputSchema: hypothesisTestProportionProblemSchema,

  outputSchema: generatedVariantsWithCanonicalProblemSchema,

  execute: async ({ inputData }) => {
    const prompt = `
Génère trois variantes en français de ce problème de test d’hypothèse pour une proportion.
...
`;
    ...
    const response =
      await hypothesisTestProportionFrenchLanguageVariantAgent.generate(
        prompt,
        {
          structuredOutput: {
...
const validateMathInvariantsStep = createStep({
  id: 'validate-hypothesis-test-proportion-french-math-invariants',
...
- parameterPreserved: la proportion populationnelle testée est préservée.
- populationPreserved: la population visée est préservée.
- successCountPreserved: le nombre de succès observés est préservé.
- sampleSizePreserved: la taille de l’échantillon est préservée.
- sampleProportionPreserved: la proportion d’échantillon est préservée, si elle est explicitement donnée.
...
const evaluateRegisterStep = createStep({
  id: 'evaluate-hypothesis-test-proportion-french-register',
...
const computeReadabilityMetricsStep = createStep({
  id: 'compute-hypothesis-test-proportion-french-readability-metrics',
...
const assessReadabilityTargetsStep = createStep({
  id: 'assess-hypothesis-test-proportion-french-readability-targets',
...
export const generateHypothesisTestProportionFrenchVariantsWorkflow =
  createWorkflow({
    id: 'generate-hypothesis-test-proportion-french-variants',
    inputSchema: hypothesisTestProportionProblemSchema,
    outputSchema: workflowOutputSchema,
  })
