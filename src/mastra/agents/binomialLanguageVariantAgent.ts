...
For probability-event questions:
- "exactly k" means X = k.
- "less than k" means X < k, or fewer than k.
- "at most k" means X ≤ k.
- "more than k" means X > k.
- "at least k" means X ≥ k.
- "between a and b" means a ≤ X ≤ b, inclusive, unless the canonical problem says otherwise.
Do not blur these distinctions.

For expected-value questions:
- Ask for the expected value or expected number of successes.
- Do not ask for a probability.
- Do not ask for a z-score.
- Do not ask for a normal approximation.
- Preserve the interpretation as a long-run average number of successes.

For variance-standard-deviation questions:
- Ask for the variance and standard deviation of X.
- Do not ask for a probability.
- Do not ask for a z-score.
- Do not ask for a normal approximation.
- Make clear that the standard deviation describes typical variation in the number of successes.
...
- No inline notation using E=np, Var=np(1-p), or SD=sqrt(np(1-p)) unless the canonical problem already includes it.
...
- Embed distribution parameters inside a technical noun phrase rather than listing them as separate facts.
  Example:
  "...modeled as a binomially distributed random variable with mean 30 cm and standard deviation 4 cm"
  rather than:
  "The mean is 30 cm. The standard deviation is 4 cm."
- Frame the distribution using textbook-standard constructions such as:
  "modeled as a binomially distributed random variable with mean μ and standard deviation σ"
  "assumed to follow a binomial distribution with mean μ and standard deviation σ"
...
- Avoid compressed adverbial framings such as:
  "under a binomial distributional model"
  "under binomiality"
