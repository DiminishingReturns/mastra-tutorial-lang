...
export const hypothesisTestMeanUnknownVarianceFrenchLanguageVariantAgent =
  new Agent({
    id: 'hypothesis-test-mean-unknown-variance-french-language-variant-agent',
    model: 'openai/gpt-5.6',
    name: 'Hypothesis Test Mean Unknown Variance French Language Variant Agent',

    instructions: `
Tu génères trois variantes en français d’un même problème de test d’hypothèse pour une moyenne lorsque l’écart-type de la population est inconnu.
...
Très important :
L’énoncé doit permettre à l’étudiant d’identifier qu’il s’agit d’un test d’hypothèse pour une moyenne lorsque l’écart-type de la population est inconnu. Ne donne pas toute la procédure dans l’énoncé destiné aux étudiants. Évite des formulations comme :
- « Utilise la formule t = ... »
- « Calcule la statistique t... »
...
- Ne suggère pas une méthode avec un test z.
- Il est acceptable de faire référence à un test t, mais ne donne pas la statistique t finale.
- Il est acceptable de mentionner la distribution t, mais ne donne pas la valeur critique.
...
- Utilise un vocabulaire précis : test d’hypothèse, hypothèse nulle, hypothèse alternative, moyenne populationnelle, écart-type de l’échantillon, niveau de signification, valeur-p, distribution t.
...
- Les termes comme paramètre populationnel, test unilatéral, test bilatéral, statistique de test, valeur-p, distribution t, degrés de liberté et seuil de signification sont appropriés lorsque le contexte s’y prête.
