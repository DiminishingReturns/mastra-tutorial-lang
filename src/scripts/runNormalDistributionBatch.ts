...
import { generateNormalProblemVariantsWorkflow } from '../mastra/workflows/generateNormalProblemVariants.workflow';
...
async function main() {
  ...
  const workflow = generateNormalProblemVariantsWorkflow;
  ...
  const outputPath = join(
    outputDirectory,
    'normal-distribution-batch-results-every-6th.json',
  );
