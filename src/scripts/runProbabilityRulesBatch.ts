...
import { generateProbabilityRulesProblemVariantsWorkflow } from '../mastra/workflows/generateProbabilityRulesProblemVariants.workflow';
async function main() {
  ...
  const workflow = generateProbabilityRulesProblemVariantsWorkflow;
  ...
  const outputPath = join(
    outputDirectory,
    'probability-rules-batch-results-every-9th.json',
  );
