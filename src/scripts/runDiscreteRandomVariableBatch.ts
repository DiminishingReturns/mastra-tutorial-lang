...
import { generateDiscreteRandomVariableProblemVariantsWorkflow } from '../mastra/workflows/generateDiscreteRandomVariableVariants.workflow';
...
async function main() {
  ...
  const workflow = generateDiscreteRandomVariableProblemVariantsWorkflow;
  ...
  const outputPath = join(
    outputDirectory,
    'discrete-random-variable-batch-results-every-6th.json',
  );
